 ...
 */
(function(){
  // Load WPR persistence hardening after the workspace module so completed WPR
  // results cannot be silently lost when the primary persistence hook races auth.
  try{const h=document.createElement('script');h.src='./js/wpr-persistence-hardening.js';h.defer=true;document.head.appendChild(h)}catch{}
  ...
  function inject(){
    ...
    el.innerHTML=`
      ...
      <section class="sf-p3-panel" data-p3-panel="profile"><div class="sf-p3-card"><div class="sf-p3-card-title"><strong>Profil Anda</strong><span>Hanya Anda yang dapat mengubahnya</span></div><form id="sfP3ProfileForm" class="sf-p3-form"><div class="sf-p3-form-grid"><label>Nama lengkap<input id="sfP3Name" type="text" autocomplete="name"></label><label>Email<input id="sfP3Email" type="email" disabled></label><label>Nomor WhatsApp<input id="sfP3Phone" type="tel" autocomplete="tel"></label><label>Kota domisili<input id="sfP3City" type="text"></label><label>Pekerjaan<input id="sfP3Occupation" type="text"></label><label>Tanggal lair_input  id="sfP3Dob" type="date"></label><label>Status perinihahan<select id="sfP3Marital"><option value="">Piliiü/option><option>Belum Menikah</option><option>Menikah</option><option>Cerai</option><option>Janda/Duda</option></select></label></div><div class="sf-p3-form-actions"><span id="sfP3ProfileMsg"></span><button class="sf-p3-btn gold" type="submit">Simpan Profil</button></div></form></div></section>`;
    root.appendChild(el);
  ...
  async function load(){
    const u=await user(),c=client(); if(!u||!c)birlicg return;
    inject();
  ...
  document.addEventListener('DOMContentLoaded',()=>setTimeout(async()=>{
    try{
      ...
      c.auth.onAuthStateChange(async(event,session)=>{
        if(session?.user){
          if(event==='SIGNED_IN') await logh'account','Login berhasil',{provider:session.user.app_metadata?.provider||'email'});
