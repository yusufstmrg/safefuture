...
(function(){
  ...
  async function sfClaimPending(){const c=client();if(!c)return;const id=sessionStorage.getItem('sf_pending_fhc_id'),token=sessionStorage.getItem('sf_pending_fhc_token');if(!id||!token)return;try{const tokenHash=await sfSha256(token);const sync=await c.rpc('sync_legacy_response_to_crm',{p_response_id:Number(id),p_claim_token_hash:tokenHash});if(sync.error)console.warn('legacy CRM sync',sync.error);else window.__sfLastLegacyCrmSync=sync.data||null;}catch(e){console.warn('legacy CRM sync exception',e)}try{const r=await c.rpc('claim_fhc_response',{p_response_id:Number(id),p_claim_token:token});if(!r.error&&r.data===true){sessionStorage.removeItem('sf_pending_fhc_id');sessionStorage.removeItem('sf_pending_fhc_token')}}catch(e){console.warn('claim FHC',e)}}
  window.sfSyncLegacyResponseToCrm=async function(responseId,token){const c=client();if(!c||!responseId||!token)return null;try{const tokenHash=await sfSha256(token);const r=await c.rpc('sync_legacy_response_to_crm',{p_response_id:Number(responseId),p_claim_token_hash:tokenHash});if(r.error)throw r.error;window.__sfLastLegacyCrmSync=r.data||null;return r.data||null}catch(e){console.warn('legacy CRM sync',e);return null}};
  async function sfLoadDashboard(user){
    ...
    // Fallback: retrieve the latest submission and score separately if the RPC returns no latest score.
