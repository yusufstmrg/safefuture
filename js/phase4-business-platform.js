...
(function(){
  ...
  async function sf4Load(){
    ...
    $('sf4FhcHistory').innerHTML=fhcs.length?fhcs.map((r,i)=>`<div class="sf4-history"><div><b>FHC ${i===0?'Terbaru':'Riwayat'}</b><small>${new Date(r.created_at||r.submitted_at).toLocaleString('id-ID')}</small></div><div style="display:flex;align-items:center;gap:8px"><strong>${r.overall_score!=null?Math.round(Number(r.overall_score))+' / 100':'—'}</strong><button type="button" onclick="window.sfProductionViewAssessment&&window.sfProductionViewAssessment('FHC','${r.id}')">Lihat hasil</button></div></div>`).join(''):'<div class="sf4-empty">Belum ada FHC tersimpan.</div>';
    $('sf4WprHistory').innerHTML=wprs.length?wprs.map((r,i)=>`<div class="sf4-history"><div><b>WPR ${i===0?'Terbaru':'Riwayat'}</b><small>${new Date(r.completed_at||r.submitted_at||r.created_at).toLocaleString('id-ID')}</small></div><div style="display:flex;align-items:center;gap:8px"><strong>${r.wpr_results?.[0]?.overall_score!=null?Math.round(r.wpr_results[0].overall_score)+' / 100':'—'}</strong><button type="button" onclick="window.sfProductionViewAssessment&&window.sfProductionViewAssessment('WPR','${r.id}')">Lihat hasil</button></div></div>`).join(''):'<div class="sf4-empty">Belum ada WPR tersimpan.</div>';
    ...
    $('sf4Reports').innerHTML=reportRows.length?reportRows.map(r=>'<div class="sf4-history"><div><b>'+esc(r.report_type||'Report')+'</b><small>'+new Date(r.created_at||r.generated_at).toLocaleString('id-ID')+' · '+esc(r.status||'ready')+'</small></div><div style="display:flex;gap:8px;align-items:center"><button type="button" onclick="window.sfProductionViewReport&&window.sfProductionViewReport(\''+esc(r.id)+'\')">Lihat</button><button type="button" onclick="window.sfProductionDownloadReport&&window.sfProductionDownloadReport(\''+esc(r.id)+'\')">Download PDF</button></div></div>').join(''):'<div class="sf4-empty">Belum ada laporan tersimpan.</div>';
    ...
    /* Final history/report controller owns the canonical actionable rendering. */
    if(typeof window.sfFinalRefresh==='function') setTimeout(()=>window.sfFinalRefresh(),0);
  }
